...
impl Package {
    pub(super) fn get_local_path(&self) -> PathBuf {
    ...
    pub(super) fn get_kiln_cfg(&self) -> Result<Option<Config>, PkgError> {
        let cfg_file = self.get_local_path().join(CONFIG_FILE);
        if !cfg_file.exists() {
            return Ok(None);
        }
...
async fn install_globally(package: &Package) -> Result<(), PkgError> {
    let package_dir = package.get_local_path();
    ...
    let res = reqwest::get(package.tag.tarball_url.clone());
...
fn copy_deps_global_to_local(pkg: &Package, dep_type: &str) -> Result<(), PkgError> {
    ...
    if let Ok(local_pkg_source_dir) = pkg.get_source_dir() {
        for f in global_source_dir.iter() {
            let f_str = match f.to_str() {
                Some(s) => s,
                None => {
                    #[cfg(debug_assertions)]
                    {
                        println!("Received `None` from OsStr.as_str() [fn resolve_adding_package]");
                    }
                    continue;
                }
            };

            if !file_ext.iter().any(|&i| f_str.ends_with(i)) {
                continue;
            }

            let p: &Path = f.as_ref();
            let new_p = local_pkg_source_dir.join(f);
            if new_p.exists() {
                eprintln!(
